...
export function PreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const cur = localStorage.getItem("kirosim:currency") as CurrencyCode | null;
      if (cur) setCurrency(cur);
    } catch {
      /* storage blocked — the default currency still works */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("kirosim:theme", theme);
    } catch {
  ...
  const setCurrencyPersist = useCallback((c: CurrencyCode) => {
    setCurrency(c);
    try {
      localStorage.setItem("kirosim:currency", c);
    } catch {
      /* storage blocked */
    }
  }, []);
