...
export async function sendMail(mail: Mail): Promise<void> {
  ...
  const from = process.env.MAIL_FROM ?? `Kirosim <${process.env.SMTP_USER ?? "info@kirosim.com"}>`;
...
export function welcomeEmail(firstName: string): Pick<Mail, "subject" | "html" | "text"> {
  const subject = "Welcome to Kirosim";
  const text = `Hi ${firstName},\n\nYour Kirosim account is ready. You can now buy travel eSIMs, top up your balance and manage everything from your wallet.\n\nSafe travels,\nThe Kirosim team`;
  return { subject, text, html: `<p>Hi ${firstName},</p><p>Your Kirosim account is ready. You can now buy travel eSIMs, top up your balance and manage everything from your wallet.</p><p>Safe travels,<br/>The Kirosim team</p>` };
}

export function resetEmail(link: string): Pick<Mail, "subject" | "html" | "text"> {
  const subject = "Reset your Kirosim password";
...
export function welcomeBackEmail(firstName: string): Pick<Mail, "subject" | "html" | "text"> {
  const subject = "Welcome back to Kirosim";
  const text = `Hi ${firstName},\n\nYou just signed in to your Kirosim account. If this was you, there's nothing to do — your eSIMs and wallet are ready in your account.\n\nIf this wasn't you, please reset your password right away.\n\nThe Kirosim team`;
  return { subject, text, html: `<p>Hi ${firstName},</p><p>You just signed in to your Kirosim account. If this was you, there's nothing to do — your eSIMs and wallet are ready in your account.</p><p>If this wasn't you, please reset your password right away.</p><p>The Kirosim team</p>` };
}
...
export function topUpEmail(firstName: string | undefined, amountCents: number, balanceCents: number): Pick<Mail, "subject" | "html" | "text"> {
  ...
  const subject = `Your Kirosim top-up receipt — ${amount}`;
  const text = `${hi}\n\nWe've added ${amount} to your Kirosim balance.\n\nAmount added: ${amount}\nNew balance: ${balance}\n\nYou can spend it on any eSIM at checkout.\n\nSafe travels,\nThe Kirosim team\n${site.company} · ${site.url}`;
  const html = `<p>${hi}</p><p>We've added <strong>${amount}</strong> to your Kirosim balance.</p><table style="border-collapse:collapse"><tr><td style="padding:2px 12px 2px 0;color:#697">Amount added</td><td>${amount}</td></tr><tr><td style="padding:2px 12px 2px 0;color:#697">New balance</td><td><strong>${balance}</strong></td></tr></table><p>You can spend it on any eSIM at checkout.</p><p>Safe travels,<br/>The Kirosim team<br/><span style="color:#697">${site.company} · ${site.url}</span></p>`;
...
export function invoiceEmail(data: InvoiceEmailData): Pick<Mail, "subject" | "html" | "text"> {
  ...
  const subject = `Your Kirosim invoice ${data.orderRef}`;
